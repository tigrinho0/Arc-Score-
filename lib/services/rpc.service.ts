...
export class RpcService {
  ...
  async getTransactionReceipt(txHash: string): Promise<any> {
    try {
      ...
      console.error(`Error getting transaction receipt ${txHash}: ${error.message}`);
      return null;
    }
  }
  ...
  async getTransactionsInBlock(blockNumber: number): Promise<any[]> {
    try {
      ...
      console.error(`Error getting transactions in block ${blockNumber}: ${error.message}`);
      return [];
    }
  }
  ...
  async getTokenBalance(tokenAddress: string, walletAddress: string): Promise<string> {
    try {
      console.log(`Fetching token balance - Token: ${tokenAddress}, Wallet: ${walletAddress}`);
      const erc20Abi = [
      ...
      try {
        ...
        console.log(`Using checksummed token address: ${checksummedTokenAddress}`);
        contract = new ethers.Contract(checksummedTokenAddress, erc20Abi, this.provider);
      } catch (checksumError: any) {
        console.warn(`Checksum failed, using token address as-is: ${checksumError.message}`);
      ...
      console.log(`✅ Token balance retrieved: ${balanceString}`);
      return balanceString;
    } catch (error: any) {
      console.error(`❌ Error getting token balance for ${walletAddress} on ${tokenAddress}: ${error.message}`);
      if (error.code) {
  ...
  async getTokenDecimals(tokenAddress: string): Promise<number> {
    try {
      ...
      console.warn(`Error getting decimals for token ${tokenAddress}, defaulting to 6 (USDC): ${error.message}`);
      return 6;
    }
  }
}
